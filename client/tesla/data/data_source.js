...
$class('tesla.data.DataSource', {
  ...
  insert: function (record) {
    if (!this.onbeforeinsert.notice(record))
      return;
    ...
    if(!id){
      return this.onerror.notice(new Error('Insert records no primary key'));
    }

    if (this.data.innerIndexOf(identifier, id))
      return this.onerror.notice(new Error('Primary key conflict'));

    mark_change(this, id);
    this.data.push(record);
    this.oninsert.notice(record);
    autoSync(this);
  },
  ...
  update: function (record) {
    if (!this.onbeforeupdate.notice(record))
      return;

    var identifier = this.identifier;
    var id = record[identifier];
    if (!id)
      return this.onerror.notice(new Error('Update records no primary key'));

    var index = this.data.innerIndexOf(identifier, id);
    if(index == -1){
      return this.onerror.notice(new Error('No need to update the data'));
    }

    mark_change(this, id);
    this.data.splice(index, 1, record);
    this.onupdate.notice(record);
    autoSync(this);
  },
  ...
  deleted: function (record) {
    if (!this.onbeforedelete.notice(record))
      return;

    var identifier = this.identifier;
    var id = record[identifier];
    if (!id)
      return this.onerror.notice(new Error('Delete records no primary key'));

    var index = this.data.innerIndexOf(identifier, id);
    if(index == -1)
      return this.onerror.notice(new Error('No need to delete data'));

    mark_change(this, id);
    this.data.splice(index, 1);
    this.ondelete.notice(record);
    autoSync(this);
  },
  ...
  loadData: function (data, total) {
    ...
    if (!this.onbeforeload.notice({ 'static': true })){
      return;
    }
    ...
    this.onload.notice(this.dataResult(o));
  },
